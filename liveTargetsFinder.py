...
def processMasscan(ipFile, domain_map, masscanOutput, masscanPath, urlOutput):
	print(Fore.BLUE + "\n - Starting masscan...")
	subprocess.run([masscanPath if masscanPath != "" else './masscan/bin/masscan', '-iL', ipFile, '-oD', masscanOutput, '--open-only', '--max-rate', '500', '-p80,443', '--max-retries', '10'])
...
def processMassDNS(targetHosts, massdnsOutput, massdnsPath, resolversPath, domainOutput, ipOutput):
	print(Fore.BLUE + "\n - Starting massdns...")
	subprocess.run([massdnsPath if massdnsPath != "" else './massdns/bin/massdns', '-c', '25', '-o', 'J', '-r', resolversPath if resolversPath != "" else './massdns/lists/resolvers.txt', '-s', '100', '-w', massdnsOutput, '-t', 'A', targetHosts])
...
if __name__ == "__main__":
	...
	if args.targetHosts:
		targetHosts = args.targetHosts
		massdnsPath = ""
		masscanPath = ""
		resolvers = ""
		dbOutputPath = ""
		if args.massdnsPath:
			massdnsPath = args.massdnsPath
		else:
			massdnsPath = "massdns/bin/massdns"
		...
		if args.masscanPath:
			masscanPath = args.masscanPath
		else:
			masscanPath = "masscan/bin/masscan"
		...
		if (args.dbOutputPath):
			dbOutputPath = args.dbOutputPath
